<template>
  <div class="vue-cropper" ref="cropper" @mouseover="scaleImg" @mouseout="cancelScale">
    ...
    <div class="cropper-drag-box" :class="{'cropper-move': move && !crop, 'cropper-crop': crop, 'cropper-modal': cropping}"
      @mousedown="startMove"
...
export default {
  ...
  props: {
    ...
    /*
      ...
      !mode可能的值 contain  cover  50px   50px 60px之类
    */
    mode: {
      type: String,
      default: "contain"
  ...
  methods: {
    ...
    checkOrientationImage(img, orientation, width, height) {
      ...
      ctx.save(); // !这里没循环执行多次，这里的ctx.save和restore感觉没啥作用
      
      // todo orientation
      canvas.width = width;
      canvas.height = height;

      ctx.drawImage(img, 0, 0, width, height); // !画出来的图片宽高 不超过2000
      ctx.restore();
      canvas.toBlob( // !toBlob和toDataURL 都可以，toBlob  异步、blob 二进制格式渲染比较快；toDataURL 同步、base64格式 渲染慢一些
        blob => {
      ...
      // this.imgs = canvas.toDataURL('image/' + this.outputType); // !其实就是base64那串string
    },
    ...
    checkedImg() {
      ...
      img.onload = () => {
        ...
        exifmin.getData(img).then(data => { // !得到的data.arrayBuffer没用过。。
          this.orientation = data.orientation || 1;
          let max = this.maxImgSize; // !默认最大2000
    ...
    startMove(e) {
      ...
      if (this.move && !this.crop) { // !this.move 这里一定为true的，没卵用   crop也一直都是false
        if (!this.canMove) {
          return false;
        }
        console.log(e.clientX,e.clientY)
        // console.log(e.touches) // !移动端
        // 开始移动
        // this.moveX = (e.clientX ? e.clientX : e.touches[0].clientX) - this.x;
        // this.moveY = (e.clientY ? e.clientY : e.touches[0].clientY) - this.y;
        this.beginMoveX = e.clientX ? e.clientX : e.touches[0].clientX // !开始移动时鼠标的X坐标
        this.beginMoveY = e.clientY ? e.clientY : e.touches[0].clientY
        this.originX = this.x // !开始移动时 记录图片的原始x轴、y轴偏移量   this.x和this.y
        this.originY = this.y
        if (e.touches) {
    ...
    moveImg(e) {
      ...
      changeX = nowX - this.beginMoveX + this.originX; // !beginMoveX开始移动时鼠标的X坐标 ,originX开始移动时图片的原始x轴偏移量 
      changeY = nowY - this.beginMoveY + this.originY;
      ...
      this.$nextTick(() => {
        ...
        this.$emit("imgMoving", {
          moving: true,
          axis: this.getImgAxis() // !得到相对容器左上角的坐标x1,x2,y1,y2
        });
        this.$emit("img-moving", {
          moving: true,
          axis: this.getImgAxis()
    ...
    scaleImg() {
      if (this.canScale) {
        window.addEventListener(this.support, this.changeSize, this.passive);  //!passive 提前告知浏览器这个事件需要阻止默认行为的,提高性能
      }
    },
    // 移出框
    cancelScale() {
    ...
    moveCrop(e, isMove) { // !
      ...
      this.$nextTick(() => {
        let cx, cy;
        let fw = nowX - this.cropX; // !cropX = x - this.cropOffsertX;  // 相当于等到一个偏移值
        let fh = nowY - this.cropY; // !cropY = y - this.cropOffsertY;
        if (isMove) {
    ...
    getImgAxis(x, y, scale) {
      ...
      switch (this.rotate) {
        case 0:
          obj.x1 = x + (this.trueWidth * (1 - scale)) / 2; // !盒子左边 到 图片左上角的点 的x偏移   (this.trueWidth * (1 - scale)) / 2这个得到缩放后的偏移量   x是主动偏移量
          obj.x2 = obj.x1 + this.trueWidth * scale; // !盒子左边 到 图片右上角的点 的x偏移
    ...
    getCropData1(cb) {
      ...
      let dx = -(this.cropOffsertX - (this.w - this.trueWidth * this.scale) / 2) // !这个更直观
      let dy = -(this.cropOffsertY - (this.h - this.trueHeight * this.scale) / 2)
      console.log(dx,dy)
    ...
    showPreview() {
      // 优化不要多次触发
      if (this.isCanShow) { // !因为watch了多个属性，属性改变就执行showPreview，这个isCanShow防止执行多次。
      ...
      <div class="show-preview" style="width: ${obj.w}px; height: ${obj.h}px; overflow: hidden">
        <div style="width: ${w}px; height: ${h}px">
          <img src=${obj.url} style="width: ${this.trueWidth}px; height: ${this.trueHeight}px; transform:scale(${scale})translate3d(${transformX}px, ${transformY}px, ${transformZ}px)rotateZ(${this.rotate * 90}deg)">
        </div>
      </div>`;
      // console.log(obj)
    ...
    reload() {
      ...
      img.onload = () => {
        ...
        let scaleBy = '' // !按宽还是高缩放
        // 判断是否需要压缩大图
        if (!this.original) { // !相当于 自适应显示contain效果
        ...
        this.$nextTick(() => {
          // !图片偏移x轴 y轴
          if (scaleBy === 'w') { // !scale = this.w / this.trueWidth;
          ...
          } else if (scaleBy === 'h') { // !scale = this.h / this.trueHeight; 这个情况是
            let deltaX = (this.trueWidth - this.trueWidth * this.scale) / 2; // ! 移动到容器壁左侧需要的距离
            let deltaZ = (this.w - this.trueWidth * this.scale) / 2; // !这是居中后，容器壁左侧到缩放后的图片的距离
            this.x = -(deltaX - deltaZ);
            ...
            this.x = this.w / 2 - this.trueWidth / 2
            this.y = this.h / 2 - this.trueHeight / 2
          }
          //console.log(scale)
          // !上面的if逻辑得到的简写就是下面的
          //this.x = -(this.trueWidth - this.trueWidth * this.scale) / 2 +(this.w - this.trueWidth * this.scale) / 2;
    ...
    //   const arr = this.mode.split(" "); // !mode可能的值 contain  cover  50px   50px 60px之类
    //   switch (arr[0]) {
    //     case "contain":
    //       if (this.trueWidth > this.w) { // !这里意味着   this.trueWidth * scale  === this.w   意味铺满 容器宽  part1
    //         // 如果图片宽度大于容器宽度
    //         scale = this.w / this.trueWidth;
    //       }
